...
export default function OfficeLogin() {
        ...
        if (timerActive && currentUser) {
            interval = setInterval(async () => {
                try {
                    ...
                    const q = query(workSessionsRef, where('status', '==', 'active'), limit(1));
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            ...
            const todayTasks = tasksData.filter((task) => {
                const createdAtDate = task.createdAt?.toDate?.().toISOString().split('T')[0];
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            const workSessionsRef = collection(db, 'users', currentUser.uid, 'workSessions');
            const q = query(workSessionsRef, where('status', '==', 'active'), orderBy('startTime', 'desc'), limit(1));
            const querySnapshot = await getDocs(q);
            if (!querySnapshot.empty) {
            ...
            const dailySummaryRef = doc(db, 'users', currentUser.uid, 'workHours', today);
        ...
        <div className='flex justify-center p-4 min-h-screen'>
            <Card className='w-full max-w-2xl bg-black/20 backdrop-blur-lg border border-white/20 text-white mt-16 h-fit'>
                <CardHeader className='text-center'>
                    <div className='flex justify-between items-center'>
                        <div className='flex-1 text-left'>
                            <Button variant='ghost' onClick={handleCheckout} className='text-red-400 hover:text-white hover:bg-red-500/20'>
                                <ExitIcon className='mr-2 h-4 w-4' />
                                Checkout ({formatTime(elapsedTime)})
                            </Button>
                        </div>
                        <div className='flex-1'>
                            <CardTitle className='text-3xl'>Daily Tasks</CardTitle>
                        </div>
                ...
                <CardContent>
                    ...
                    {loading ? (
                        <div className='text-center text-gray-300 py-4'>Loading tasks...</div>
                    ) : (
                        <div className='space-y-3'>
                            {tasks.length > 0 ? (
                                ...
                                <p className='text-center text-gray-400 py-4'>No tasks for today. Add one to get started!</p>
                            )}
                        </div>
                    )}
                </CardContent>
