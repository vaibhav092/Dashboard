...
export default function ClientsList() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setEmployees(employeesSnap.docs.map(doc => ({ id: doc.id, name: doc.data().name })));
            } catch (error) {
    ...
    const getPlanStatus = (planEndDate) => {
        if (!planEndDate) return { status: 'Unknown', className: 'bg-gray-200 text-gray-800' };
    ...
    if (loading) {
        return <div className="flex justify-center p-8"><Loader2 className="animate-spin text-blue-500" size={48} /></div>;
    }
        ...
        <div className={`container mx-auto py-8`}>
            <Card className="shadow-xl border-gray-200 rounded-lg">
                ...
                <CardContent className="p-6">
                    {clients.length === 0 ? (
                        <p className="text-center text-gray-500 text-lg py-10">No clients added yet. Click "Add Client" to get started!</p>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {clients.map(client => {
                                const status = getPlanStatus(client.planEndDate);
                                return (
                                    <Card
                                        key={client.id}
                                    ...
                                    >
                                            ...
                                            <p>
                                                ...
                                                {client.assignedEmployees && client.assignedEmployees.length > 0 ? (
                                                    client.assignedEmployees.map(empId => {
                                                        ...
                                                        return emp ? (
                                                            <span key={empId} className="inline-block bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs mr-1 mb-1">
                                                                {emp.name}
                                                            </span>
                                                        ) : null;
                                                    })
                                        ...
                                        <CardFooter className="p-4 pt-0 text-xs text-gray-500 border-t border-gray-100 flex justify-between items-center">
                                            {client.planEndDate && (
                                                <p>
                                                    Ends:{' '}
                                                    {client.planEndDate.toDate ? client.planEndDate.toDate().toLocaleDateString() : new Date(client.planEndDate).toLocaleDateString()}
                                                </p>
                                            )}
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <Button variant="outline" size="sm">
                                                        <UserPlus className="mr-2 h-4 w-4" /> Assign/Remove
                                                    </Button>
                                                </DropdownMenuTrigger>
                                                <DropdownMenuContent>
                                                    {employees.length === 0 ? (
                                                        ...
                                                        <>
                                                            {employees.map(employee => (
                                                                <DropdownMenuItem
                                                                    key={employee.id}
                                                                    onClick={() => assignEmployee(client.id, employee.id)}
                                                                    disabled={client.assignedEmployees?.includes(employee.id)}
                                                                >
                                                                    <UserPlus className="mr-2 h-4 w-4" /> Assign {employee.name}
                                                                </DropdownMenuItem>
                                                            ))}
                                                            {client.assignedEmployees && client.assignedEmployees.length > 0 && (
                                                                <>
                                                                    ...
                                                                    {client.assignedEmployees.map(empId => {
                                                                        ...
                                                                        return emp ? (
                                                                            <DropdownMenuItem
                                                                                key={`remove-${empId}`}
                                                                                onClick={() => removeEmployee(client.id, empId)}
                                                                            >
                                                                                <UserMinus className="mr-2 h-4 w-4 text-red-500" /> Remove {emp.name}
                                                                            </DropdownMenuItem>
                                                                        ) : null;
                                                                    })}
                                                                </>
                    ...
                    <Dialog open={!!selectedClient} onOpenChange={() => setSelectedClient(null)}>
                        <DialogContent className="sm:max-w-[425px] bg-white p-6">
                            <DialogHeader className="border-b pb-4 mb-4 border-gray-200">
                                ...
                                <DialogDescription className="text-gray-600">Detailed information for this client.</DialogDescription>
                            </DialogHeader>
                            {selectedClient && (
                                <div className="grid gap-4 py-4 text-gray-700">
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Country:</Label>
                                        <span className="col-span-2">{selectedClient.country}</span>
                                    </div>
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">State/Region:</Label>
                                        <span className="col-span-2">{selectedClient.state || 'N/A'}</span>
                                    </div>
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Timezone:</Label>
                                        <span className="col-span-2">{selectedClient.timezone}</span>
                                    </div>
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Business Type:</Label>
                                        <span className="col-span-2">{selectedClient.businessType}</span>
                                    </div>
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Plan:</Label>
                                        <span className="col-span-2">{selectedClient.plan}</span>
                                    </div>
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Plan End Date:</Label>
                                        <span className="col-span-2">
                                            {selectedClient.planEndDate ? (
                                                selectedClient.planEndDate.toDate ? selectedClient.planEndDate.toDate().toLocaleDateString() : new Date(selectedClient.planEndDate).toLocaleDateString()
                                            ) : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Plan Status:</Label>
                                        <span className={`col-span-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium ${getPlanStatus(selectedClient.planEndDate).className}`}>
                                            {getPlanStatus(selectedClient.planEndDate).status}
                                        </span>
                                    </div>
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Tech Stack:</Label>
                                        <span className="col-span-2">{selectedClient.techStack?.join(', ') || 'N/A'}</span>
                                    </div>
                                    {selectedClient.otherTechStack && (
                                        <div className="grid grid-cols-3 items-center gap-4">
                                            <Label className="text-right font-medium">Other Tech:</Label>
                                            <span className="col-span-2">{selectedClient.otherTechStack}</span>
                                        </div>
                                    )}
                                    <div className="grid grid-cols-3 items-center gap-4">
                                        <Label className="text-right font-medium">Assigned Employees:</Label>
                                        <span className="col-span-2">
                                            {selectedClient.assignedEmployees && selectedClient.assignedEmployees.length > 0 ? (
                                                selectedClient.assignedEmployees.map(empId => {
                                                    const emp = employees.find(e => e.id === empId);
                                                    return emp ? (
                                                        <span key={empId} className="inline-block bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs mr-1 mb-1">
                                                            {emp.name}
                            ...
                            <DialogFooter className="pt-4 border-t border-gray-200">
                                <Button onClick={() => setSelectedClient(null)} className="bg-blue-600 hover:bg-blue-700 text-white">Close</Button>
                            </DialogFooter>
                        </DialogContent>
                    </Dialog>
                </CardContent>
